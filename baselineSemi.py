...
def mysetup(myobj, discrete=True):
    ...
    myobj.add_design_variable('base_outer_diameter',2.1, 40.0)
    ...
    myobj.add_design_variable('auxiliary_freeboard',2.0, 15.0)
    myobj.add_design_variable('auxiliary_section_height',1e-1, 50.0)
    myobj.add_design_variable('auxiliary_outer_diameter',1.1, 40.0)
    myobj.add_design_variable('auxiliary_wall_thickness',1e-3, 5e-1)
    myobj.add_design_variable('auxiliary_ballast_heave_box_diameter',0.0, 50.0)
    myobj.add_design_variable('auxiliary_ballast_heave_box_height',0.0, 20.0)
    myobj.add_design_variable('auxiliary_ballast_heave_box_location',0.0, 1.0)
    myobj.add_design_variable('pontoon_outer_diameter', 1.0, 10.0)
    myobj.add_design_variable('pontoon_wall_thickness', 1e-2, 1.0)
    myobj.add_design_variable('base_pontoon_attach_lower',0.0, 0.5)
    myobj.add_design_variable('base_pontoon_attach_upper',0.5, 1.0)
    ...
    myobj.add_design_variable('auxiliary_stiffener_web_height', 1e-2, 1.0)
    myobj.add_design_variable('auxiliary_stiffener_web_thickness', 1e-3, 5e-1)
    myobj.add_design_variable('auxiliary_stiffener_flange_width', 1e-2, 5.0)
    myobj.add_design_variable('auxiliary_stiffener_flange_thickness', 1e-3, 5e-1)
    myobj.add_design_variable('auxiliary_stiffener_spacing', 1e-1, 1e2)
    myobj.add_design_variable('auxiliary_permanent_ballast_height', 1e-1, 50.0)
    myobj.add_design_variable('radius_to_auxiliary_column', 5.0, 100.0)
    if discrete:
        #myobj.add_design_variable('number_of_mooring_connections', 3, 4)
        myobj.add_design_variable('mooring_lines_per_connection', 1, 3)
        #myobj.add_design_variable('number_of_auxiliary_columns', 3, 4)
    ...
    myobj.params['number_of_auxiliary_columns'] = 3
